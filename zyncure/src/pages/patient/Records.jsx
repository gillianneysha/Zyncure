...
// Helper function - downloading a file
const downloadFile = async (filePath, filename) => {
  try {
    console.log('Downloading file:', filePath, filename);

    // Use Supabase's download method instead of fetch
    ...
    console.log('File data received, creating download...');
...
// FIXED: Improved categorizeFiles function
function categorizeFiles(files, fileShares, currentUserId) {
  ...
  for (const fs of fileShares) {
    ...
    sharesByFileId[fs.file_id] = sharesByFileId[fs.file_id] || [];
    sharesByFileId[fs.file_id].push(fs);
  }

  const mine = [];
  const shared_with_me = [];
  const shared_with_others = [];
  const allSet = new Set();

  for (const file of files) {
    const shares = sharesByFileId[file.id] || [];
    const isOwner = file.owner_id === currentUserId;

    // Check if file is actively shared with current user
    const isSharedWithUser = shares.some(
      (fs) => fs.is_active && fs.shared_with_id === currentUserId
    );

    // Check if file is shared with others (only relevant for owned files)
    const isSharedWithOthers = isOwner && shares.some(
      (fs) => fs.is_active && fs.shared_with_id && fs.shared_with_id !== currentUserId
    );

    // Only include files that the user should see
            ...
            <div className="absolute right-0 top-full w-36 bg-white border border-gray-200 rounded-lg shadow-lg z-20">
              {isDoctorSharedNotePdf ? (
                <button
                  onClick={() => {
                    setDropdown(false);
                    downloadFile(file.file_path, name);
                  }}
                  className="flex items-center gap-2 w-full px-4 py-2 text-gray-700 hover:bg-indigo-50 transition-colors"
                >
                ...
                <>
                  ...
                  {/* FIXED: Replace anchor tag with button that calls downloadFile */}
                  <button
                    onClick={() => {
                      setDropdown(false);
...
export default function Records({ currentUserId: propUserId, isPatient: propIsPatient }) {
  ...
  async function fetchFolders() {
    ...
    // Get shared folders via file_shares (where shared_with_id = currentUserId, is_active, share_type = 'folder')
    const { data: sharedFolderShares = [] } = await supabase
      .from("file_shares")
      .select("folder_id")
    ...
    const sharedFolderIds = sharedFolderShares.map(row => row.folder_id).filter(Boolean);

    let sharedFolders = [];
    if (sharedFolderIds.length > 0) {
      const { data: foldersFromShares = [] } = await supabase
        .from("folders")
        .select("*")
        .in("id", sharedFolderIds);
      sharedFolders = foldersFromShares;
    }

    // Deduplicate
    const allFolders = [...ownedFolders, ...sharedFolders];
    const dedupedFolders = allFolders.filter(
      (folder, idx, arr) => arr.findIndex(f2 => f2.id === folder.id) === idx
    );
  ...
  // FIXED: Enhanced fetchFilesAndShares function with better error handling
  async function fetchFilesAndShares() {
    if (!currentUserId) return;
    console.log('Fetching files and shares for user:', currentUserId);
    setLoading(true);

    try {
      // 1. Fetch all files owned by the user
      ...
      // 2. Fetch all ACTIVE file shares where user is recipient
      ...
      console.log('Active shares with user:', activeShares);

      // 3. Get files that are shared with the user
      const sharedFileIds = activeShares.map(share => share.file_id).filter(Boolean);
      let sharedFiles = [];
      if (sharedFileIds.length > 0) {
        const { data: filesFromShares = [], error: sharedFilesError } = await supabase
          .from("medical_files")
          .select("*")
          .in("id", sharedFileIds);
      ...
      // 4. Fetch ALL file_shares that involve the current user's files or shares
      const ownedFileIds = ownedFiles.map(file => file.id);
      let allFileShares = [...activeShares]; // Start with active shares where user is recipient

      // Get shares for files owned by the user
      if (ownedFileIds.length > 0) {
        const { data: sharesForOwned = [], error: ownedSharesError } = await supabase
          .from("file_shares")
          .select("*")
          .in("file_id", ownedFileIds);

        if (ownedSharesError) {
          console.error("Error fetching shares for owned files:", ownedSharesError);
        } else {
          // Add shares for owned files, avoiding duplicates
          const existingShareIds = new Set(allFileShares.map(s => s.id));
          const newShares = sharesForOwned.filter(s => !existingShareIds.has(s.id));
          allFileShares = [...allFileShares, ...newShares];
        }
      }

      // 5. Combine and deduplicate all files
      const allFiles = [...ownedFiles, ...sharedFiles];
      const dedupedFiles = allFiles.filter(
        (file, idx, arr) => arr.findIndex(f2 => f2.id === file.id) === idx
      );

      console.log('Deduped files count:', dedupedFiles.length);
      console.log('All file shares count:', allFileShares.length);
      ...
      setFileShares(allFileShares);
    } catch (error) {
      console.error("Error in fetchFilesAndShares:", error);
    } finally {
      setLoading(false);
    }
  }
      ...
      if (!tierStatus.can_upload_files) {
        return {
          ...
          message: `Storage limit reached! You've used ${tierStatus.storage_used_mb.toFixed(
            1
          )}MB of your ${tierStatus.storage_limit_mb
  ...
  // FIXED: Enhanced rename/delete logic with better error handling and proper share management
  const doRename = async (newName) => {
    ...
    try {
      if (renameModal.type === "file") {
        ...
        // Allow patients to rename files shared with them OR files they own
        const canRename = file.owner_id === currentUserId ||
          (isPatient && fileShares.some(share =>
        ...
        if (!canRename) {
          ...
          setSuccessModal({
            open: true,
            title: "Rename Failed",
            message: "You can only rename files you own or files shared with you.",
          });
          return;
        }
  ...
  // FIXED: Enhanced delete function with proper handling for doctor-shared files
  const doDelete = async () => {
    if (!deleteModal.open) return;

    console.log('Starting delete operation for:', deleteModal);
    ...
    try {
      if (deleteModal.type === "file") {
        ...
        console.log('File owner ID:', file.owner_id);

        const isOwner = file.owner_id === currentUserId;

        // Get all ACTIVE shares for this file where current user is recipient
        const relevantShares = fileShares.filter(share =>
          share.file_id === file.id &&
          share.shared_with_id === currentUserId &&
          share.is_active === true
        );

        console.log('Relevant active shares:', relevantShares);

        const isSharedWithUser = relevantShares.length > 0;

        // Allow patients to delete both owned files and files shared with them
        const canDelete = isOwner || (isPatient && isSharedWithUser);

        console.log('Can delete?', canDelete, 'Is owner?', isOwner, 'Is shared?', isSharedWithUser);

        if (!canDelete) {
          setSuccessModal({
            open: true,
            title: "Delete Failed",
            message: "You can only delete files you own or files shared with you.",
          });
          return;
        }

        if (isOwner) {
          console.log('Deleting owned file...');

          // If user owns the file, delete it completely
          // First, delete from storage if file path exists
          ...
          // Then delete from database (this will cascade delete shares)
          ...
          console.log('File deletion result:', dbError);
          ...
          if (dbError) {
            ...
            console.log('Successfully deleted file, refreshing data...');

            // Force refresh the data
            await Promise.all([
              fetchFilesAndShares(),
              storageInfo ? fetchStorageInfo() : Promise.resolve()
            ]);
        ...
        } else if (isPatient && isSharedWithUser) {
          // Patient deleting shared file - actually delete the file completely
          console.log('Patient deleting shared file completely...');
          ...
          // Then delete from database (this will cascade delete all shares)
          const { error: dbError } = await supabase
            .from("medical_files")
            .delete()
            .eq("id", deleteModal.id);

          console.log('Shared file deletion result:', dbError);
          ...
          if (dbError) {
            ...
            console.log('Successfully deleted shared file, refreshing data...');
        ...
        // Close preview if the deleted/removed file was being previewed
  ...
  let displayedFiles = [];
  if (activeFolderId) {
    displayedFiles = (categorized[allFilter] || categorized["all"]).filter(
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="relative flex items-center" ref={dropdownRef}>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-2 py-2 rounded-md hover:bg-indigo-200 transition-colors ${viewMode === "list"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-2 py-2 rounded-md hover:bg-indigo-200 transition-colors ${viewMode === "grid"
              ? "bg-indigo-200 text-mySidebar"
              : "text-mySidebar"
              }`}
          >
        ...
        <div className="inline-flex items-center gap-2 text-xs text-gray-500">
          <div className="w-12 h-1 bg-gray-200 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${storageInfo.storage_limit_mb === -1
                  ? 0
                  : Math.min(
                    (storageInfo.storage_used_mb /
                      storageInfo.storage_limit_mb) *
                    100,
                    100
                  )
                  }%`,
              ...
              <div className="flex flex-row flex-wrap gap-1 mb-4 items-stretch">
                ...
                <div className="relative flex" ref={allDropdownRef}>
                  ...
                  >
                    <span>
                      {
                        ALL_FILTERS.find((f) => f.value === allFilter)?.label ??
                        "All"
                      }
                    </span>
                    <ChevronDown size={15} className="ml-1 text-[#e37859]" />
            ...
            <div className={`bg-white rounded-lg w-full relative shadow-2xl border flex flex-col ${showNotesSidebar ? "max-w-4xl md:flex-row" : "max-w-md"}`}>
              <div className="flex-1 min-w-0 p-4 flex flex-col">
                ...
                <div className="mt-4 text-right">
                  <button
                    onClick={() => downloadFile(previewFile.file_path, previewFile.name)}
              ...
              {/* Sidebar visible only when theres an attached note */}
              {showNotesSidebar && (
