      ...
      if (
        shares.some(
          (fs) =>
            fs.is_active &&
            fs.shared_with_id &&
            fs.shared_with_id !== currentUserId
        )
      ) {
...
// NEW: Fetch files AND file_shares and categorize in-memory
async function fetchFilesAndShares() {
  if (!currentUserId) return;
  setLoading(true);

  // 1. Fetch all files visible to the user (owned OR shared)
  //  - Owned by user
  ...
  //  - Shared with user (via file_shares, is_active)
  ...
  const sharedFileIds = sharedFileShares.map(row => row.file_id).filter(Boolean);
  ...
  // 2. Fetch all file_shares for files owned by current user (to filter 'shared with others')
  const { data: allFileShares = [] } = await supabase
    .from("file_shares")
    .select("*")
    .or(
      `shared_with_id.eq.${currentUserId},owner_id.eq.${currentUserId}`
    )
    .eq("is_active", true);

  // Deduplicate all files (owned + shared)
  ...
  setFileShares(allFileShares); // Store all shares for filtering!
  setLoading(false);
}
