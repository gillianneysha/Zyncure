...
const PeriodTracker = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('loggedDates');
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed)) {
          setLoggedDates(parsed);
        }
      } catch (e) {
    ...
    const fetchLoggedDates = async () => {
      ...
      if (!error && data && Array.isArray(data)) {
        setLoggedDates(data);
        localStorage.setItem('loggedDates', JSON.stringify(data));
      } else if (error) {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setLoggedDates((prev) => {
        ...
        const updated = alreadyExists ? prev : [...prev, dataToSave];
        localStorage.setItem('loggedDates', JSON.stringify(updated));
        return updated;
      });
  ...
  const getTileContent = ({ date: d }) => {
    ...
    const match = loggedDates.find((entry) => {
