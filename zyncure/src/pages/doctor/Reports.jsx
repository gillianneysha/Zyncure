...
import { Plus, Menu, Grid3X3, List, MoreHorizontal, User, Clock, Calendar, FileText, Download, Eye } from 'lucide-react';
...
const PatientsPage = () => {
  ...
  const [sharedSymptomsFiles, setSharedSymptomsFiles] = useState([]);
  ...
  const [activeTab, setActiveTab] = useState('patients'); // 'patients' or 'shared-symptoms'
  ...
  const loadConnectedPatients = async () => {
    try {
      ...
      console.log('Loaded patient folders:', patientFolders);
  ...
  // ========================================
  // SHARED SYMPTOMS FILES LOADING
  // ========================================
  const loadSharedSymptomsFiles = async () => {
    try {
      ...
      console.log('Loaded shared symptoms files:', files);
      setSharedSymptomsFiles(files);
      
    } catch (error) {
      console.error('Error loading shared symptoms files:', error);
      setSharedSymptomsFiles([]);
    } finally {
  ...
  // ========================================
  // RENDER SHARED SYMPTOMS FILE AS FOLDER
  // ========================================
  const renderSharedSymptomsFile = (file) => (
    <div
      key={file.name}
      className="relative group cursor-pointer"
    ...
    >
      ...
      <div className="relative">
        {/* Folder back part */}
        <div className="w-20 h-16 bg-green-500 rounded-t-lg transform rotate-1 absolute top-0 left-2"></div>
        
        {/* Main folder */}
        <div className="w-24 h-20 bg-green-600 rounded-lg relative overflow-hidden group-hover:bg-green-700 transition-colors duration-200">
          {/* Folder tab */}
          <div className="absolute -top-2 left-2 w-8 h-4 bg-green-600 rounded-t-md group-hover:bg-green-700 transition-colors duration-200"></div>
        ...
        <button
          onClick={(e) => {
            e.stopPropagation();
            setShowDropdown(showDropdown === file.name ? null : file.name);
          }}
          className="absolute -top-1 -right-1 p-1 bg-white rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-gray-50"
        >
          <MoreHorizontal className="w-4 h-4 text-gray-600" />
      ...
      {/* File info below folder */}
      <div className="mt-3 text-center">
        <h3 className="font-medium text-gray-900 text-sm truncate group-hover:text-green-600 transition-colors">
          {file.name}
        </h3>
        <p className="text-xs text-gray-500 mt-1">
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-teal-600">
            {activeTab === 'patients' ? 'Patients' : 'Shared Symptoms'}
          </h1>
          <p className="text-gray-600 mt-1">
            {activeTab === 'patients' 
              ? `${patients.length} connected patient${patients.length !== 1 ? 's' : ''}`
              : `${sharedSymptomsFiles.length} shared file${sharedSymptomsFiles.length !== 1 ? 's' : ''}`
            }
          </p>
        </div>
      ...
      <div className="flex space-x-1 mb-6">
        ...
        <button
          onClick={() => setActiveTab('shared-symptoms')}
          className={`px-4 py-2 rounded-lg font-medium transition-colors ${
            activeTab === 'shared-symptoms'
              ? 'bg-teal-100 text-teal-600'
              : 'text-gray-500 hover:text-gray-700'
          }`}
        >
      ...
      {/* Filters - only show for patients tab */}
      {activeTab === 'patients' && (
      ...
      {activeTab === 'patients' ? (
        ...
        // Shared Symptoms Files
        isLoadingFiles ? (
          <div className="text-center py-12">
            <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-teal-600"></div>
            <p className="text-gray-500 mt-2">Loading shared symptoms files...</p>
          </div>
        ) : sharedSymptomsFiles.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6">
            {sharedSymptomsFiles.map(renderSharedSymptomsFile)}
          </div>
        ) : (
          <div className="text-center py-12">
            <FileText className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-900 mb-2">No shared symptoms files found</h3>
            <p className="text-gray-500 mb-6">
              No files are currently available in the shared symptoms bucket
            </p>
          </div>
        )
      )}
