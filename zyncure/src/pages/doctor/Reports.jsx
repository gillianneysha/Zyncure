...
const PatientsPage = () => {
  ...
  try {
    ...
    const patientFolders = await Promise.all(
      (data || []).map(async (connection) => {
        const sharedFiles = await loadSharedFilesForPatient(
  ...
  const getFilteredPatients = () => {
    ...
    if (filterType === "recent") {
      ...
      filtered = filtered.filter(
        (patient) => new Date(patient.lastAccessed) >= oneWeekAgo
      );
    }
    ...
    if (filterHistory === "recent") {
      ...
      filtered = filtered.filter(
        (patient) => new Date(patient.connectedAt) >= oneMonthAgo
      );
    }
    ...
    >
      ...
      <div className="mt-3 text-center">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {patient.recordsCount} shared file
          {patient.recordsCount !== 1 ? "s" : ""}
        </p>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {patients.length} connected patient
            {patients.length !== 1 ? "s" : ""}
          </p>
        </div>
