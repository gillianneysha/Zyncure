...
const DoctorAppointments = () => {
  ...
  const [reasonAction, setReasonAction] = useState(null); // 'cancel' or 'reschedule'
  const [reasonAppointmentId, setReasonAppointmentId] = useState(null);
  ...
  const loadAppointments = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } =
        await doctorAppointmentService.getDoctorAppointments(
          dateString,
          statusFilter
        );
  ...
  const loadAppointmentsForCalendar = useCallback(async () => {
    try {
      ...
      const { data, error } =
        await doctorAppointmentService.getDoctorAppointmentsRange(
  ...
  const handleNextPage = () => {
    ...
    const totalPages = Math.ceil(
      selectedDateAppointments.length / appointmentsPerPage
    );
  ...
  const handleConfirmAppointment = async (appointmentId) => {
    setLoading(true);
    try {
      const { error } = await doctorAppointmentService.updateAppointmentStatus(
        appointmentId,
        "confirmed"
      );
      if (error) {
  ...
  const handleRescheduleAppointment = async (appointmentId, reason = "") => {
    setLoading(true);
    try {
      const { error } = await doctorAppointmentService.rescheduleAppointment(
        appointmentId,
        reason
      );
      if (error) {
  ...
  const handleCancelAppointment = async (appointmentId, reason = "") => {
    setLoading(true);
    try {
      const { error } = await doctorAppointmentService.cancelAppointment(
        appointmentId,
        reason
      );
      if (error) {
  ...
  const handleViewDetails = async (appointment) => {
    ...
    try {
      const { data, error } = await doctorAppointmentService.getPatientDetails(
  ...
  const showCancelReasonModal = (appointmentId) => {
    setReasonAppointmentId(appointmentId);
    setReasonAction("cancel");
    setShowReasonModal(true);
  };

  const showRescheduleReasonModal = (appointmentId) => {
    setReasonAppointmentId(appointmentId);
    setReasonAction("reschedule");
    setShowReasonModal(true);
  };

  const handleReasonSubmit = () => {
    if (reasonAction === "cancel") {
      handleCancelAppointment(reasonAppointmentId, cancelReason);
    } else if (reasonAction === "reschedule") {
      handleRescheduleAppointment(reasonAppointmentId, rescheduleReason);
    }
  };
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasAppointments =
        ...
        appointmentsByDate[dateString].length > 0;
  ...
  const confirmedCount = selectedDateAppointments.filter(
                ...
                paginatedAppointments.map((appointment) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button
                        onClick={() =>
                          showRescheduleReasonModal(appointment.id)
                        }
                        disabled={loading}
                      ...
                      <button
                        onClick={() => showCancelReasonModal(appointment.id)}
                        disabled={loading}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full">
            ...
            <div className="p-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Reason for{" "}
                {reasonAction === "cancel" ? "cancellation" : "rescheduling"}{" "}
                (optional)
              </label>
              <textarea
                value={
                  reasonAction === "cancel" ? cancelReason : rescheduleReason
                }
                onChange={(e) => {
                  if (reasonAction === "cancel") {
