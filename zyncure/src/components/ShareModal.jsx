    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-xl px-8 py-7 w-full max-w-xs text-center shadow-2xl border relative">
        <div className="font-bold text-lg mb-2 text-gray-800">
          ...
          <span className="text-[#15907C]">{doctorName}</span>?
        </div>
...
// Sharing Limit Modal
function SharingLimitModal({ open, message, onClose }) {
  if (!open) return null;
  if (typeof window === "undefined" || typeof document === "undefined")
    return null;
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-xl px-8 py-7 w-full max-w-sm text-center shadow-2xl border relative">
        <div className="font-bold text-lg mb-2 text-gray-800">Sharing Limit Reached</div>
        <div className="mb-6 text-gray-700 text-sm">{message}</div>
        <button
          className="w-full bg-[#E36464] hover:bg-[#c64a4a] text-white rounded-full py-2 font-semibold text-base transition-colors"
          onClick={onClose}
...
const fetchUserTierStatus = async (userId) => {
  try {
    const { data, error } = await supabase
      .from("user_tier_status")
      .select(
        "current_tier, can_upload_files, can_add_symptoms, can_add_healthcare_providers"
      )
      .eq("user_id", userId)
      .single();
...
const ShareModal = ({ isOpen, onClose, item, currentUserId }) => {
  ...
  const checkSharingLimits = async () => {
    if (maxDoctorsPerShare === -1) return true; // Unlimited for premium/pro
    ...
    try {
      ...
      const currentActiveShares = currentShares?.length || 0;
      const totalAfterSharing = currentActiveShares + selectedDoctorIds.length;
  ...
  const handleShare = async () => {
    ...
    for (const doctorId of selectedDoctorIds) {
      ...
      if (existing && existing.length > 0) {
        ...
        if (!item?.type) {
          errorDoctors.push({
            doctorId,
            reason: "ShareModal error: item.type is missing!",
          });
          allSuccessful = false;
          continue;
        }
    ...
    if (allSuccessful) {
      ...
      const expDate = expirationDate
        ...
        : null;
      if (expDate) expiryString = ` until ${expDate}`;
      setFeedbackModal({
        open: true,
        title: "Shared Successfully!",
        message: `Your record has been shared successfully with ${sharedNames.join(
          ", "
        )}${expiryString}.`,
      });
    } else {
