...
// 1. First, add this helper function at the top (after other utility functions)
/**
...
`)
      ...
      const transformedData = [{
        ...
        requested_time: data.requested_time, // instead of convertTo12Hour(data.requested_time)
        date: data.requested_date, // For backward compatibility
        time: data.requested_time, // instead of convertTo12Hour(data.requested_time)
      ...
      // FIXED: Use the display time format for the notification message
      const displayTime = normalizedTime.includes('AM') || normalizedTime.includes('PM')
        ? normalizedTime
        : convertTo12Hour(normalizedTime);

      // FIXED: For the appointment_date in metadata, ensure proper ISO format
      const appointmentDateTime = `${data.requested_date}T${padSeconds(
        normalizedTime.includes('AM') || normalizedTime.includes('PM')
          ? convertTo24Hour(normalizedTime)  // Convert 12-hour to 24-hour for ISO
          : normalizedTime  // Already in 24-hour format
      )}`;
  ...
  async getUserAppointments(userId = null, options = {}) {
    try {
      ...
      if (options.startDate) {
        ...
        query = query.or(`appointment_date.gte.${dateString},and(appointment_date.is.null,requested_date.gte.${dateString})`);
      }
      ...
      query = query.order('status', { ascending: false }) // This will put 'requested' before 'confirmed' alphabetically, so we reverse it
      ...
      const transformedData = (data || []).map(apt => {
        ...
        if (apt.status === 'confirmed' && apt.appointment_date && apt.appointment_time) {
      ...
      transformedData.sort((a, b) => {
        const aDate = new Date(`${a.date}T${a.time && a.time.includes('AM') || a.time && a.time.includes('PM') ? convertTo24Hour(a.time) : '00:00'}`);
        const bDate = new Date(`${b.date}T${b.time && b.time.includes('AM') || b.time && b.time.includes('PM') ? convertTo24Hour(b.time) : '00:00'}`);
        return aDate - bDate;
      });
        ...
        try {
          await supabase
            ...
            .insert({
              ...
              message: `Patient has cancelled their appointment request for ${existingAppointment.requested_date} at ${existingAppointment.requested_time.includes('AM') || existingAppointment.requested_time.includes('PM') ? existingAppointment.requested_time : convertTo12Hour(existingAppointment.requested_time)}`,
              data: {
      ...
      try {
        await supabase
          ...
          .insert({
            ...
            message: `Patient has requested to reschedule their appointment from ${originalAppointment.requested_date} ${originalAppointment.requested_time.includes('AM') || originalAppointment.requested_time.includes('PM') ? originalAppointment.requested_time : convertTo12Hour(originalAppointment.requested_time)} to ${newAppointmentData.requested_date} ${newAppointmentData.requested_time}`,
            data: {
              original_appointment_id: originalAppointmentId,
